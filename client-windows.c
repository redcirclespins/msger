...
#include <stdlib.h>
#include <string.h>
#include <winsock2.h>  // Changed from <unistd.h> to <winsock2.h> for Windows socket API
#include <ws2tcpip.h>  // Added to use inet_pton for IP address conversion
...
int main(int argc,char** argv){
    ...
    WSADATA wsaData;  // Added for Winsock initialization
    ...
    // Initialize Winsock
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {  // Changed: Added Winsock initialization
        std_error("WSAStartup failed.\n");
    }
    ...
    sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);  // Changed: Socket creation for Windows
    if(sockfd == INVALID_SOCKET) {  // Changed: Checking for socket creation failure
        error("socket error");
    }
    ...
    // Convert host to IP address and store in serv_addr.sin_addr.s_addr
    if (inet_pton(AF_INET, argv[1], &serv_addr.sin_addr) <= 0) {  // Changed: Using inet_pton for Windows
        struct hostent* host = gethostbyname(argv[1]);
        if (host == NULL) {
            std_error("Invalid host\n");
        }
        memcpy(&serv_addr.sin_addr.s_addr, host->h_addr_list[0], host->h_length);
    }

    if (connect(sockfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) == SOCKET_ERROR) {  // Changed: connect function returns SOCKET_ERROR in Windows
        error("connect error");
    }
    ...
    u_long mode = 1; // Non-blocking mode (Windows specific)
    ioctlsocket(sockfd, FIONBIO, &mode);  // Changed: Using ioctlsocket for non-blocking socket on Windows
    ...
    while(1){
        ...
        if(select(sockfd+1, &readfds, NULL, NULL, NULL) == SOCKET_ERROR) {  // Changed: select function returns SOCKET_ERROR in Windows
            error("select error");
        }
        // msgs from server
        ...
        if(FD_ISSET(0, &readfds)){
            ...
            if(strcmp(buffer, ".quit") == 0)
                ...
                while (*path_start == ' ') path_start++; // skip space
                ...
                // Send control message: .file filename filesize
                char control[256];
    ...
    SSL_free(ssl);
    closesocket(sockfd);  // Changed: closesocket for Windows
    SSL_CTX_free(ctx);

    WSACleanup();  // Added: Cleanup Winsock
    return 0;
}
